...
export const capturedTextBlocksExpiration = (textBlocks) => {
    ...
    for (let index = 0; index < textBlocks.length; index++) {
        ...
        for (let index2 = 0; index2 < textBlocks[index].components.length; index2++){
            ...
            if ((expiredFound === false) &&
            (textBlocks[index].components[index2].value.toLowerCase() === "exp" || textBlocks[index].components[index2].value.toLowerCase() === "expiry" || textBlocks[index].components[index2].value.toLowerCase() === "expdate" ||
            textBlocks[index].components[index2].value.toLowerCase() === "exp:" || textBlocks[index].components[index2].value.toLowerCase() === "exp.date" || textBlocks[index].components[index2].value.toLowerCase() === "expiry:" ||
            textBlocks[index].components[index2].value.toLowerCase() === "exp.")){    
                expiredFound = true;
        }
            if (expiredFound === true) {
                console.log("Kevin1: " + textBlocks[index].components[index2].value);
                if (expPat1.test(textBlocks[index].components[index2].value) || expPat7.test(textBlocks[index].components[index2].value)) {
                    console.log("Kevin: Return 1");
                    let newExpDate = textBlocks[index].components[index2].value + " ";
                    return newExpDate;
                }
                else if (expPat2.test(textBlocks[index].components[index2].value.toLowerCase()) || expPat5.test(textBlocks[index].components[index2].value.toLowerCase())) {
